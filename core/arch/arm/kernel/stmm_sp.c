...
static struct efi_hob_handoff_info_table *build_stmm_boot_hob_list(
		vaddr_t sp_addr, uint32_t sp_size, uint32_t *hob_table_size)
{
	...
	ret = create_guid_hob(hob_table, &ns_buf_guid,
			sizeof(struct efi_mmram_descriptor), &guid_hob_data);
	if (ret) {
...
static TEE_Result load_stmm(struct stmm_ctx *spc)
{
	...
	ffa_boot_info_header = (struct ffa_boot_info_header_1_1 *) stmm_sec_buf_addr;
	ffa_boot_info_header->signature = FFA_BOOT_INFO_SIGNATURE;
	ffa_boot_info_header->version = FFA_VERSION_1_2;
	ffa_boot_info_header->desc_size = sizeof(struct ffa_boot_info_1_1);
	ffa_boot_info_header->desc_count = 1;
	ffa_boot_info_header->desc_offset = sizeof(struct ffa_boot_info_header_1_1);
	ffa_boot_info_header->reserved = 0x00;
	ffa_boot_info_header->blob_size = (ffa_boot_info_header->desc_offset +
			(ffa_boot_info_header->desc_size * ffa_boot_info_header->desc_count));

	ffa_boot_info_desc = (struct ffa_boot_info_1_1 *) (stmm_sec_buf_addr +
	...
	res = vm_set_prot(&spc->uctx, stmm_image_addr, uncompressed_size_roundup,
